...
<div class="row">
    <div class="col">
        <div class="btn-group" role="group" aria-label="Basic example">
            <button title="<?= Kohana::message('correction', 'correct') ?>" disabled name="btnCorrect" id="btnCorrect" onclick="goActionButton('<?= URL::site('correction/correct') ?>');" type="button" class="btn btn-primary"><?= HTML::image(Kohana::message('images', 'icons.correct')) ?></button>
...
<table class="table" id="list">
    ...
    <tbody>
        <?php
        if (count($wordings)) {
            foreach ($wordings as $value) {
                ?>
                <tr onclick="setSelectedLine('#list',<?= $value->id ?>, ['#btnCorrect']);">
                    <th scope="row"><?= $value->id ?></th>
                    <td><?= $value->student->person->name ?></td>
                    <td><?= $value->student->institution->institution ?></td>
                    <td><?= Date::formatted_time($value->insertdate, 'd/m/Y') ?></td>
