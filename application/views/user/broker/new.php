...
<div class="col-md-8 order-md-1">   
    <form name="frmUserNew" action="<?= $actionForm ?>" method="post" accept-charset="uft-8" class="needs-validation" novalidate>
        ...
        <br/>
        <div class="mb-3">
            <label for="txtName"><?= Kohana::message('role', 'role.singular') ?></label><br/>
            <?php
            $i = 1;
            foreach ($roles as $role) {
                ?>
                <div class="form-check">
                    <input tabindex="<?= $i++ ?>" class="form-check-input" name="typeUser" type="radio" id="typeUser" value="<?= $role->name ?>" required>
        ...
        <div class="mb-3">
            ...
            <input tabindex="4" name="txtName" type="text" class="form-control" id="txtName" placeholder="<?= Kohana::message('people', 'name') ?>" value="" required>
            <div class="invalid-feedback"><?= Kohana::message('warning', 'people.invalidName') ?></div>
        </div>
        <div class="mb-3">
            <label for="email"><?= Kohana::message('user', 'email') ?></label>
            <input tabindex="5" type="email" name="email" class="form-control" id="email" placeholder="you@example.com" required />
            <div class="invalid-feedback"><?= Kohana::message('warning', 'invalidEmail') ?></div>
        </div>
        <div class="mb-3">
            <label for="username"><?= Kohana::message('user', 'username') ?></label>
            <input tabindex="6" name="username" type="text" class="form-control" id="username" placeholder="<?= Kohana::message('user', 'username') ?>" required />
