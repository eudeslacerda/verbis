<div class="col h1"><?= Kohana::message('page', 'new.female') . " " . Kohana::message('theme', 'theme.singular') ?></div>
<div class="col-md-8 order-md-1">
    <form name="frmUserNew" enctype="multipart/form-data" action="<?= $actionForm ?>" method="post" accept-charset="uft-8" class="needs-validation" novalidate>
        ...
        <br/>
        <div class="mb-3">
            <label for="txtTheme"><?= Kohana::message('theme', 'theme.singular') ?></label>
            <input tabindex="1" name="txtTheme" type="text" class="form-control" id="txtTheme" placeholder="<?= Kohana::message('theme', 'theme.singular') ?>" required />
            <div class="invalid-feedback"><?= Kohana::message('warning', 'requiredField') ?></div>
        </div>
        <div class="mb-3">
            <label for="txtValidity"><?= Kohana::message('theme', 'validity') ?></label>
            <input tabindex="2" name="txtValidity" type="date" class="form-control" id="txtValidity" placeholder="<?= Kohana::message('theme', 'Validity') ?>" required />
            <div class="invalid-feedback"><?= Kohana::message('warning', 'requiredField') ?></div>
        </div>
        <div class="mb-3">
            <label for="txtDescription"><?= Kohana::message('theme', 'description') ?></label>
            <textarea name="txtDescription" class="form-control" id="txtValidity" required></textarea>
            <div class="invalid-feedback"><?= Kohana::message('warning', 'requiredField') ?></div>
        </div>
    </form>
</div>
